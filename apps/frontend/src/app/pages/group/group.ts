...
export class GroupView {
  ...
  protected readonly group = computed(
    () => this.store.groups().find((g) => g.id === this.groupId()) ?? null,
  );
  ...
  private readonly transactions = computed(() =>
    this.store.transactionsOf(this.groupId()).filter((t) => !t.deleted),
  );
  ...
  protected readonly sections = computed<DateSection[]>(() => {
    ...
    for (const transaction of [...this.transactions()].sort((a, b) =>
      b.billed_at.localeCompare(a.billed_at),
    )) {
