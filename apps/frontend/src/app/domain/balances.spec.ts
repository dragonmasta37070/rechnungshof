...
describe('balancesFor', () => {
  ...
  it('honours an absolute split', () => {
    ...
    const bob = account('Bob');
    const transactions = [
      expense(30, alice.id, { [bob.id]: 10, [alice.id]: 20 }, { split_mode: 'absolute' }),
    ];
...
describe('netByPerson', () => {
  ...
  it('nets opposing debts across two groups down to the difference', () => {
    ...
    expect(netted[0].groups.sort()).toEqual(['Flat', 'Ski']);
  });
  ...
  it('hides counterparties that cancel out exactly', () => {
    ...
    const a = ledger(
      ...
      'A',
      [meA, bobA],
      [expense(40, bobA.id, { [meA.id]: 1, [bobA.id]: 1 })],
      meA.id,
    );
    ...
    const b = ledger(
      ...
      'B',
      [meB, bobB],
      [expense(40, meB.id, { [meB.id]: 1, [bobB.id]: 1 })],
      meB.id,
    );
  ...
  it('ignores settlement edges the user is not part of', () => {
    ...
    const l = ledger(
      ...
      'Trip',
      [me, bob, carol],
      [expense(30, carol.id, { [bob.id]: 1, [carol.id]: 1 })],
      me.id,
    );
  ...
  it('sorts by magnitude so the largest debt is first', () => {
    ...
    const g1 = ledger(
      ...
      'G1',
      [me1, small],
      [expense(10, small.id, { [me1.id]: 1, [small.id]: 1 })],
      me1.id,
    );
    ...
    const g2 = ledger(
      ...
      'G2',
      [me2, big],
      [expense(200, big.id, { [me2.id]: 1, [big.id]: 1 })],
      me2.id,
    );
