...
export function validateSplit(mode: SplitMode, shares: Record<number, number>, value: number): SplitValidation {
 ...
 */
export function respread(mode: SplitMode, accountIds: number[], value: number): Record<number, number> {
    if (accountIds.length === 0) {
        return {};
    }
